...
class Array
{
	...
	public void merge(Array that, Array theOther)
	{
		int index1 = 0;
		int index2 = 0;
		while(index1 < that.list.size() && index2 < theOther.list.size()){
			if(that.list.get(index1) > theOther.list.get(index2)) {
				this.list.add(theOther.list.get(index2));
				index2++;
		}
		
		else {
			this.list.add(that.list.get(index1));
			index1++;
		}
		if(index2 < theOther.list.size()) {
			this.list.add(theOther.list.get(index2));
		}
